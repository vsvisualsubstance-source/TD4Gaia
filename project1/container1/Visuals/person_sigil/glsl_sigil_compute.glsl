...
void main() {
	...
	float energy = uMod.x;
	float angle = t * 6.28318530718 + uTime * (0.2 + energy * 0.8);
	vec3 pos;

	if (shapeMode == 0) {
		// ring: steady orbit at fixed radius, gentle vertical bob
		float radius = 0.55;
		pos = vec3(radius * cos(angle), 0.15 * sin(uTime * 0.5 + t * 6.28318530718), radius * sin(angle));
	} else if (shapeMode == 1) {
		// spiral: radius grows with point index, winds outward
		float radius = 0.15 + 0.5 * t;
		float spiralAngle = angle * 3.0;
		pos = vec3(radius * cos(spiralAngle), (t - 0.5) * 0.6, radius * sin(spiralAngle));
	} else {
		// cluster: loose stable-jittered blob, no strict ordering
		float jr = hashFloat(t * 13.1);
		float ja = hashFloat(t * 27.7) * 6.28318530718;
		float radius = 0.25 + 0.25 * jr;
		pos = vec3(radius * cos(ja), (jr - 0.5) * 0.4, radius * sin(ja));
	}

	P[id] = pos;

	vec3 col = hsv2rgb(vec3(uHue, 0.6, 1.0));
	Color[id] = vec4(col, 1.0);
	PointScale[id] = 0.5 + energy * 1.0;
}
