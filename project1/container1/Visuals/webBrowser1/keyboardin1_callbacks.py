...
def onKey(dat, keyInfo):
	if keyInfo.state:
		if keyInfo.key.isdigit():
			op('webrender1').sendKey(ord(keyInfo.key), char=keyInfo.character, alt=keyInfo.alt, ctrl=keyInfo.ctrl, shift=keyInfo.shift, cmd=keyInfo.cmd)
		else:
			op('webrender1').sendKey(keyInfo.key, char=keyInfo.character, alt=keyInfo.alt, ctrl=keyInfo.ctrl, shift=keyInfo.shift, cmd=keyInfo.cmd)
	return
