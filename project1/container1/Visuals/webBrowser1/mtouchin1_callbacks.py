...
def onDown(dat, event):
	op('webrender1').interactTouch(event.u, event.v, radiusx=event.contactx, radiusy=event.contacty, start=event.id)
	return

def onMove(dat, event):
	op('webrender1').interactTouch(event.u, event.v, radiusx=event.contactx, radiusy=event.contacty, move=event.id)
	return

def onUp(dat, event):
	op('webrender1').interactTouch(event.u, event.v, radiusx=event.contactx, radiusy=event.contacty, end=event.id)
	return
